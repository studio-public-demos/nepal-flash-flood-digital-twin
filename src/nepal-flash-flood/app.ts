...
type ScenarioData = {
  runs: SimulationRun[];
  provenance: SimulationRun["provenance"];
};

type GeoJsonFeature = {
  properties: InfrastructureAsset & { kind: string };
  geometry: { type: string; coordinates: number[] | number[][] };
};

type ObservedEvidenceFeature = {
  properties: {
    id: string;
    name: string;
    kind: "satellite_scene" | "observed_community" | "observed_tributary";
    phase?: string;
    sensor?: string;
    datetime?: string;
    gsd?: number | null;
...
type TerrainSection = {
  ...
  samples: Array<TerrainSample & { offsetM: number }>;
};

type CrossValleySample = TerrainSample & {
  offsetM: number;
  centerIndex: number;
};
...
const state = {
  runs: [] as SimulationRun[],
  currentRun: null as SimulationRun | null,
  referenceRun: null as SimulationRun | null,
  infrastructure: [] as InfrastructureAsset[],
  observedEvidence: [] as ObservedEvidenceFeature[],
  time: 60,
  playing: false,
  speed: 1,
  mode: "explore",
  ...
  viewer: null as any,
  waterEntities: [] as any[],
  flowEntities: [] as any[],
  observedEntities: [] as any[],
  journeyEntities: [] as any[],
  assetEntities: [] as any[],
  terrainSamples: [] as TerrainSample[],
  terrainSections: [] as TerrainSection[],
  terrainStatus: "Terrain profile pending",
  flowCanvas: null as HTMLCanvasElement | null,
  flowContext: null as CanvasRenderingContext2D | null,
...
async function loadData() {
  ...
  const scenarioData = (await scenarioRes.json()) as ScenarioData;
  const geojson = (await infraRes.json()) as { features: GeoJsonFeature[] };
  const evidence = evidenceRes.ok ? (await evidenceRes.json()) as { features: ObservedEvidenceFeature[] } : { features: [] };
  ...
  state.infrastructure = geojson.features
...
async function loadRuntimeConfig() {
  ...
  try {
    ...
    // Local config is optional. Deployed builds can inject config by other means.
  }
}

function getCesiumToken(): string {
  const metaToken = document.querySelector<HTMLMetaElement>('meta[name="cesium-ion-token"]')?.content.trim();
...
async function initTerrainProvider(Cesium: any, token: string) {
  if (token) {
    try {
...
async function initImageryProvider(Cesium: any, token: string) {
  if (token) {
    try {
...
async function ensureCesium(): Promise<any | null> {
...
async function sampleCorridorTerrain() {
  ...
  try {
    const sampled = await withTimeout<any[]>(Cesium.sampleTerrainMostDetailed(state.viewer.terrainProvider, cartographics), 8000, "Terrain sampling timed out");
    const crossValleySamples: CrossValleySample[] = sampled.map((point: any, index: number) => ({
    ...
    state.terrainSections = centerline.flatMap((point, index) => {
      const sectionSamples = crossValleySamples
        .filter((sample) => sample.centerIndex === index)
...
function offsetFromCenterline(centerline: number[][], index: number, offsetM: number): [number, number] {
...
function offsetFromProgress(centerline: number[][], progress: number, offsetM: number): [number, number] {
  ...
  state.assetEntities = state.infrastructure.map((asset) => {
    ...
    return state.viewer.entities.add({
      ...
      label: {
        ...
        heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
        disableDepthTestDistance: Number.POSITIVE_INFINITY,
        scaleByDistance: new Cesium.NearFarScalar(25000, 1, 180000, 0.35),
      },
      properties: asset,
    });
  });
  state.viewer.screenSpaceEventHandler.setInputAction((click: any) => {
...
function drawJourneyAnnotations(corridor: number[][]) {
    ...
    state.journeyEntities.push(
      state.viewer.entities.add({
        ...
        label: {
          ...
          heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
          disableDepthTestDistance: Number.POSITIVE_INFINITY,
          scaleByDistance: new Cesium.NearFarScalar(28000, 1, 180000, 0.28),
        },
        properties: {
          ...stage,
...
function polygonRings(coordinates: any): number[][][] {
  if (!Array.isArray(coordinates)) return [];
  if (typeof coordinates[0]?.[0]?.[0] === "number") return coordinates as number[][][];
  if (typeof coordinates[0]?.[0]?.[0]?.[0] === "number") return (coordinates as number[][][][]).flat();
  return [];
}
      ...
      state.observedEntities.push(
        state.viewer.entities.add({
          ...
          label: {
            ...
            heightReference: Cesium.HeightReference.CLAMP_TO_GROUND,
            disableDepthTestDistance: Number.POSITIVE_INFINITY,
            scaleByDistance: new Cesium.NearFarScalar(22000, 1, 150000, 0.25),
          },
          properties: feature.properties,
        }),
      );
...
function setEntityVisibility() {
  for (const entity of state.assetEntities) {
    ...
    entity.show =
      (kind === "bridge" && state.layers.bridges) ||
      (kind === "settlement" && state.layers.settlements) ||
      (kind === "road" && state.layers.roads) ||
      (kind === "critical_facility" && state.layers.critical);
  }
...
function corridorFootprint(centerline: number[][], widthMeters: number): number[][] {
...
function terrainWaterEdgeOffset(section: TerrainSection, side: -1 | 1, stageRiseM: number, halfMinimum: number, halfMaximum: number) {
  ...
  const samples = section.samples
    .filter((sample) => (side < 0 ? sample.offsetM <= 0 : sample.offsetM >= 0))
    .sort((a, b) => Math.abs(a.offsetM) - Math.abs(b.offsetM));
...
function terrainConstrainedFootprint(centerline: number[][], stageRiseM: number, minimumWidthM: number, maximumWidthM: number): number[][] {
...
function terrainSampleCount(): number {
  return state.terrainSections.reduce((total, section) => total + section.samples.length, 0) || state.terrainSamples.length;
}

function drawInundationBand(Cesium: any, points: number[][], depthM: number, color: any, id: string) {
...
function drawWaterSpine(Cesium: any, centerline: number[][], frame: ReturnType<typeof frameAt>) {
...
function setupFluidCanvas() {
  const canvas = $("#flowCanvas") as HTMLCanvasElement | null;
  if (!canvas) return;
...
function visibleFlowWindow(centerline: number[][]) {
...
function resetFlowParticle(particle: FlowParticle, centerline: number[][], front: number) {
...
function projectToCanvas(Cesium: any, lon: number, lat: number, heightM: number) {
...
function drawSurgeFront(ctx: CanvasRenderingContext2D, Cesium: any, centerline: number[][], frame: ReturnType<typeof frameAt>, nowSeconds: number) {
  const { front } = visibleFlowWindow(centerline);
...
function updateSurgeNarrative(centerline: number[][]) {
...
function drawFluidOverlay(nowMs: number) {
  ...
  const dt = Math.min(0.08, Math.max(0.01, (nowMs - state.lastFlowMs) / 1000 || 0.016));
...
function renderMetrics() {
  ...
  $("#metrics").innerHTML = run.metrics
    .map((metric) => `<div class="metric"><span>${metric.label}</span><strong>${metric.value} ${metric.unit}</strong><em>${metric.classification}</em></div>`)
    .join("");
...
function inspectAsset(asset: InfrastructureAsset) {
...
function assetPanel(asset: InfrastructureAsset, exposure: AssetExposure) {
...
function scenarioFromControls(): SimulationScenario {
  ...
  const value = <T extends HTMLInputElement | HTMLSelectElement>(id: string) => $<T>(`#${id}`).value;
  return {
    ...base,
    id: "visitor-scenario",
    ...
    breachMechanism: value("breachMechanism") as SimulationScenario["breachMechanism"],
    breachDurationMinutes: Number(value("breachDuration")),
    relativeBreachWidth: value("breachWidth") as SimulationScenario["relativeBreachWidth"],
    rainfallMultiplier: Number(value("rainfall")),
    antecedentFlow: value("antecedentFlow") as SimulationScenario["antecedentFlow"],
    debrisPercent: Number(value("debris")),
    channelRoughness: value("roughness") as SimulationScenario["channelRoughness"],
    bridgeCondition: value("bridgeCondition") as SimulationScenario["bridgeCondition"],
    secondaryBlockage: $<HTMLInputElement>("#secondaryBlockage").checked,
  };
}
...
async function runPresetScenario(id: string) {
  if (!state.engine) return;
...
function bindControls() {
  ...
  $("#speed").addEventListener("change", (event) => {
    state.speed = Number((event.target as HTMLSelectElement).value);
  });
