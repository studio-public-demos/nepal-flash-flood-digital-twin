...
- Planet/Source Cooperative STAC acquisition footprints and metadata for pre-event and post-event imagery.
- OpenStreetMap communities and named tributary reaches near the corridor where source geometry is available, plus clearly labelled representative demo assets where authoritative geometry is not bundled.
...
- Explicit separation between S0, the approximately 100 Mm3 August 26 representative event replay, and S1-S7 2-5 Mm3 barrier-lake what-if scenarios.
- Scenario release hydrographs, scenario-specific arrival curves, terrain-constrained representative flood footprints, depth, velocity, and relative hydraulic hazard rendering.
...
- The public build does not yet include ANUGA, HEC-RAS, BASEMENT, or another solver-backed 2D shallow-water output.
